...
$root = dirname(__DIR__); // sube desde api/ a la raíz
...
foreach ($folders as $folder) {
    ...
    if ($path === 'assets') {
        if (!file_exists("$path/files.json") || filemtime($path) > filemtime("$path/files.json")) {
            generateAssetsJson("./$path/");
        }
    } else {
        if (!file_exists("$path/folders.json") || filemtime($path) > filemtime("$path/folders.json")) {
            generateFoldersJson("./$path/");
        }
    }
}
?>
...
<head>
    ...
    <title>(API) Gestor de Archivos por Tabs</title>
